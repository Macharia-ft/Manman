...
module.exports = {
  ...
  checkMediaUploadEligibility: async (req, res) => {
    try {
      const authHeader = req.headers.authorization;
      if (!authHeader) {
        return res.status(401).json({ success: false, message: "Missing token" });
      ...
      const uploadPeriodDays = isFreePlan ? 30 : 7; // 30 days for free, 7 days for premium

      // Check last media update
      ...
      let canUpload = true;
      let daysUntilNextUpload = 0;

      if (lastUpdate && lastUpdate.length > 0) {
        const lastUpdateDate = new Date(lastUpdate[0].requested_at);
        const now = new Date();
        const daysSinceLastUpdate = Math.floor((now - lastUpdateDate) / (1000 * 60 * 60 * 24));
        
        if (daysSinceLastUpdate < uploadPeriodDays) {
          canUpload = false;
          daysUntilNextUpload = uploadPeriodDays - daysSinceLastUpdate;
        }
      }
      ...
      console.error("❌ Media eligibility check error:", error.message);
      res.status(500).json({
        success: false,
