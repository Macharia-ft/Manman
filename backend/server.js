...
// ✅ API route to check media upload status and remaining time
app.get('/api/user/media-upload-status', async (req, res) => {
  try {
    const token = req.headers.authorization?.split(" ")[1];
    if (!token) {
      return res.status(401).json({ success: false, message: "Missing token" });
    ...
    const restrictionDays = userSubscription === 'premium' ? 7 : 30;
    ...
    if (lastUpdate && !lastUpdateError) {
      ...
      if (daysSinceLastUpdate < restrictionDays) {
        canUpload = false;
        remainingDays = restrictionDays - daysSinceLastUpdate;
        nextUploadDate = new Date(lastUpdateDate.getTime() + (restrictionDays * 24 * 60 * 60 * 1000));
      }
    }
    ...
    res.json({
      ...
      remainingDays,
      nextUploadDate,
      lastUploadDate: lastUpdate?.requested_at || null
    });
  } catch (error) {
